...
struct Avy_View_State {
    ...
    void operator=(const std::initializer_list<int>) {}
};

// @NOTE scroll_cursor_line / center_view
function void scroll_cursor_line(Application_Links *app, int view_pos, View_ID view_id = -1) {
...
function String_Const_u8 * avy_generate_keys(Arena *arena, int key_count) {
...
function Avy_Selection_Result avy_get_single_character_selection_from_user(Application_Links *app, b32 allow_only_avy_key_list_chars) {
...
function Avy_Pair * avy_get_key_selection_from_user(Application_Links *app, Avy_Pair *pairs, int count) {
...
function void avy_goto_text(Application_Links *app, b32 single_character) {
...
CUSTOM_COMMAND_SIG(avy_goto_char) {
    ...
    Managed_Scope view_scope = view_get_managed_scope(app, view_id);
    Avy_View_State *avy_state = scope_attachment(app, view_scope, view_avy_state, Avy_View_State);
    avy_state->action = avy_action_none;
}
...
// @note: Avy rendering
//
function void avy_render(Application_Links *app, View_ID view_id, Buffer_ID buffer_id, Text_Layout_ID text_layout_id, Face_ID face_id, Rect_f32 view_region, f32 roundness, ARGB_Color argb_background, ARGB_Color argb_foreground) {
...
function void avy_render(Application_Links *app, View_ID view_id, Text_Layout_ID text_layout_id, Rect_f32 view_region) {

...
//~ TODO: Auto snippet don't add when typing (conflict with vim?)

#ifndef FCODER_PENGUIN_AUTO_SNIPPET_
#define FCODER_PENGUIN_AUTO_SNIPPET_
...
function void penguin_write_text(Application_Links *app, String_Const_u8 insert){
...
CUSTOM_COMMAND_SIG(penguin_write_text_input)
CUSTOM_DOC("Inserts whatever text was used to trigger this command.")
{
    User_Input in = get_current_input(app);
    String_Const_u8 insert = to_writable(&in);
    penguin_write_text(app, insert);
}

CUSTOM_COMMAND_SIG(penguin_write_text_and_auto_indent)
...
function void penguin_auto_snippet(Application_Links *app, View_ID view_id, Buffer_ID buffer, Face_ID face_id, Text_Layout_ID text_layout_id) {
	...
	if (global_snippet_cursor_set) {
		...
		// // turn off auto snippet in line comment
		// if (penguin_has_line_comment(app, buffer, current_line_number)) {
		// 	global_snippet_cursor_set = false;
		// }
...
#endif // FCODER_PENGUIN_AUTO_SNIPPET_
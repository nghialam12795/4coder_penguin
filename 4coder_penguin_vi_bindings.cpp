...
#ifndef FCODER_PENGUIN_VI_BINDINGS_
#define FCODER_PENGUIN_VI_BINDINGS_
/* USAGE:
    Override some defines here for configuration:
...
#define VIM_JUMP_HISTORY_SIZE 50
#endif
...
#define VIM_CURSOR_ROUNDNESS 0.7f
#endif
...
// NOTE: vim_query functions all look at the _current_ input via
// vim_get_current_key(), and they always consume keys they parse successfully.
...
internal Vim_Motion_Result vim_motion_character_seek_internal(
                                                              Application_Links* app,
                                                              View_ID view,
                                                              Buffer_ID buffer,
                                                              i32 motion_count,
...
CUSTOM_DOC("[vim] Inserts text and auto-indents the line on which the cursor sits if any of the text contains 'layout punctuation' such as ;:{}()[]# and new lines, and substitutes words according to the abbreviations added with the vim_add_abbreviation function.")
{
  ...
  if (sizeof(VIM_ESCAPE_SEQUENCE) >= 1) {
...
/// Hooks
/// ----------------------------------------------------------------------
function void vim_draw_character_i_bar(Application_Links *app, Text_Layout_ID layout, i64 pos, ARGB_Color color){
  Rect_f32 rect = text_layout_character_on_screen(app, layout, pos);
  rect.x1 = rect.x0 + 6.f;
  draw_rectangle(app, rect, 0.f, color);
}

static void vim_draw_character_i_bar(Application_Links *app, Text_Layout_ID layout, i64 pos, FColor color){
...
function void vim_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id, Buffer_ID buffer, Text_Layout_ID text_layout_id, Rect_f32 rect, Frame_Info frame_info) {
  ...
  penguin_paint_functions(app, buffer, text_layout_id);
  ...
  // NOTE(rjf): Error annotations
  {
    String_Const_u8 name = string_u8_litexpr("*compilation*");
    Buffer_ID compilation_buffer = get_buffer_by_name(app, name, Access_Always);
    penguin_render_error_annotation(app, buffer, text_layout_id, compilation_buffer);
  }
  
  // Package: divider_comments
  penguin_render_divider_comment(app, buffer, view_id, text_layout_id);
...
internal f32 DrawFileBarMode(Application_Links * App, Face_ID FaceID, Rect_f32 Bar, String_Const_u8 BufferName, b32 IsBuffer, f32 DigitAdvance)
{
  ...
  if (!IsBuffer)
  {
    ...
    if (global_keyboard_macro_is_recording)
    {
      ...
      if (IsSameBuffer)
      {
        BackgroundColor = fcolor_argb(finalize_color(defcolor_bar, 6));
...
internal f32 DrawFileBarBufferName(Application_Links * App, Buffer_ID BufferID, Face_ID FaceID, Rect_f32 Bar, f32 StartPoint, f32 DigitAdvance)
{
  Scratch_Block Scratch(App);
  ...
  draw_rectangle_fcolor(App, BarPartition, 0, BackgroundColor);
  BarPartition.x1 = BarPartition.x0 = DrawFileBarTriangles(App, Bar, PaddingColor, StartPoint, 1);
...
internal void DrawFileBarBufferInfo(Application_Links * App, View_ID ViewID, Buffer_ID BufferID, Face_ID FaceID, Rect_f32 Bar, f32 DigitAdvance, f32 RenderBreakpoint, b32 IsBuffer)
{
  Scratch_Block Scratch(App);
  ...
  if (!IsBuffer)
  {
    ...
    push_fancy_stringf(Scratch, &Line, DarkTextColor, "Time %1.lld:%1.lld", ltm->tm_hour, ltm->tm_min);
...
function void vim_draw_file_bar(Application_Links *app, View_ID view_id, Buffer_ID buffer, Face_ID face_id, Rect_f32 bar) {
  ...
  String_Const_u8 BufferName = push_buffer_unique_name(app, scratch, buffer);
  // NOTE(sammynilla): Check if the scratch is non file buffer
  b32 IsBuffer = false;
  if (!string_match(BufferName, string_u8_litexpr("*scratch*")))
  {
    if (string_match_insensitive(string_prefix(BufferName, 1), string_u8_litexpr("*")))
    {
      IsBuffer = true;
    }
  }
  
  Face_Metrics FaceMetrics = get_face_metrics(app, face_id);
  f32 DigitAdvance = FaceMetrics.decimal_digit_advance;
  
  f32 NextRenderStartPoint = 0;
  NextRenderStartPoint = DrawFileBarMode(app, face_id, bar, BufferName, IsBuffer, DigitAdvance);
  
  f32 RenderBreakpoint = DrawFileBarBufferName(app, buffer, face_id, bar, NextRenderStartPoint, DigitAdvance);
  
  DrawFileBarBufferInfo(app, view_id, buffer, face_id, bar, DigitAdvance, RenderBreakpoint, IsBuffer);
...
function void vim_render_caller(Application_Links *app, Frame_Info frame_info, View_ID view_id) {
  ...
  if (global_config.show_line_number_margins){
    Rect_f32_Pair pair = layout_colby_line_number_margin(app, buffer, region, digit_advance);
  ...
  if(global_config.show_line_number_margins) {
    if(use_relative_line_number_mode) {
      ...
      draw_colby_line_number_margin(app, view_id, buffer, face_id, text_layout_id, line_number_rect);
    }
  }
...
BUFFER_HOOK_SIG(vim_begin_buffer) {
  ...
  *map_id_ptr = vim_mapid_normal;
  ...
  //     vim_echo_alert(app, "Warning: Virtual Whitespace is not yet properly supported by 4coder_penguin.");
...
function void vim_setup_default_mapping(Application_Links* app, Mapping *mapping, Vim_Key vim_leader) {
  ...
  VimBind(vim_motion_line_start_textual,                          vim_key(KeyCode_H, KeyCode_Control));
  ...
  VimBind(interactive_open,                                    vim_leader, vim_key(KeyCode_B), vim_key(KeyCode_O));
  VimBind(interactive_open_or_new,                             vim_leader, vim_key(KeyCode_B), vim_key(KeyCode_E));
...
#endif // !FCODER_PENGUIN_VI_BINDINGS_